...
const statusConfig: Record<ToolStatus, { icon: string; label: string; cls: string }> = {
  installed:       { icon: '✅', label: 'Installed',     cls: 'text-emerald-400' },
  outdated:        { icon: '⚠️', label: 'Outdated',      cls: 'text-amber-400'  },
  'not-installed': { icon: '❌', label: 'Not installed', cls: 'text-slate-500'  },
  installing:      { icon: '⏳', label: 'Installing',    cls: 'text-primary-400'},
  error:           { icon: '🔴', label: 'Error',         cls: 'text-red-400'    },
}

export default function DashboardPage() {
  const navigate  = useNavigate()
  const { tools, platform, getState, detectTools, uninstallTool, exportConfig } = useTools()
  ...
  const installed = platformTools.filter(t => getState(t.id).status === 'installed')
  const outdated  = platformTools.filter(t => getState(t.id).status === 'outdated')
  const errored   = platformTools.filter(t => getState(t.id).status === 'error')

  const displayed = filter === 'all'
    ? platformTools
    : platformTools.filter(t => getState(t.id).status === filter)
  ...
  async function handleExport() {
    ...
    a.href = url; a.download = 'devtoolshub-config.json'; a.click()
      ...
      <div className="px-6 py-4 border-b border-surface-700 bg-surface-900 flex-shrink-0">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleRefresh} disabled={refreshing} className="btn-ghost text-xs disabled:opacity-50">
              {refreshing ? '⏳ Scanning...' : '🔄 Refresh'}
            </button>
            <button onClick={() => navigate('/installer')} className="btn-primary text-xs">
              📦 Install More
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: 'Installed',         val: `${installed.length}/${platformTools.length}`, cls: 'text-slate-100' },
          ...
          {(['all', 'installed', 'outdated', 'not-installed', 'error'] as const).map(f => (
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-medium transition-colors
                ${filter === f ? 'bg-primary-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-surface-700'}`}
            >
              {f === 'all' ? `All (${platformTools.length})` : f === 'not-installed' ? 'Not Installed' : f.charAt(0).toUpperCase() + f.slice(1)}
              {f !== 'all' && (
                <span className="ml-1 opacity-60">
                  ({platformTools.filter(t => getState(t.id).status === f).length})
                </span>
              )}
            </button>
          ))}
              ...
              {displayed.map(tool => {
                ...
                return (
                  <tr key={tool.id} className="hover:bg-surface-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="badge bg-surface-700 text-slate-400 capitalize">
                        {tool.category.replace('-', ' ')}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`font-medium ${cfg.cls}`}>{cfg.icon} {cfg.label}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {state.status === 'not-installed' && (
                        <button onClick={() => navigate('/installer')} className="btn-ghost text-xs py-1 px-2">Install</button>
                      )}
                      {state.status === 'outdated' && (
                        <button className="btn-ghost text-xs py-1 px-2 text-amber-400">Update</button>
                      )}
                      {state.status === 'installed' && (
                        <div className="inline-flex gap-1">
                          ...
                          <button
                            onClick={() => handleUninstall(tool.id, tool.name)}
