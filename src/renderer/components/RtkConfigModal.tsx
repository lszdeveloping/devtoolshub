...
const AGENTS = [
  { id: 'claude', label: 'Claude Code', cmd: 'rtk init -g' },
  { id: 'codex', label: 'Codex (OpenAI)', cmd: 'rtk init -g --codex' },
  { id: 'gemini', label: 'Gemini CLI', cmd: 'rtk init -g --gemini' },
  { id: 'cursor', label: 'Cursor', cmd: 'rtk init --agent cursor' },
  { id: 'windsurf', label: 'Windsurf', cmd: 'rtk init --agent windsurf' },
  { id: 'cline', label: 'Cline / Roo Code', cmd: 'rtk init --agent cline' },
  { id: 'kilocode', label: 'Kilo Code', cmd: 'rtk init --agent kilocode' },
  { id: 'antigravity', label: 'Google Antigravity', cmd: 'rtk init --agent antigravity' },
  { id: 'opencode', label: 'OpenCode', cmd: 'rtk init -g --opencode' },
]
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-surface-900 border border-surface-700 rounded-xl shadow-2xl w-full max-w-md mx-4">
        <div className="px-5 py-4 border-b border-surface-700 flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-slate-100 inline-flex items-center gap-1.5">
              <Terminal className="h-4 w-4" />
              Configure RTK
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">Select which AI tools to activate RTK for</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 transition-colors text-lg leading-none">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        {results ? (
          <div className="p-5 space-y-2">
            {results.map((r) => {
              ...
              return (
                <div
                  key={r.agent}
                  className={`flex items-center gap-3 px-3 py-2 rounded-lg border ${r.success ? 'bg-emerald-950/30 border-emerald-800/40' : 'bg-red-950/30 border-red-800/40'}`}
                >
          ...
          <>
              ...
              {AGENTS.map((agent) => (
                <label
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-colors border
                    ${selected.has(agent.id) ? 'bg-primary-900/30 border-primary-700/40' : 'hover:bg-surface-800 border-transparent'}`}
                >
                  <input
                    type="checkbox"
                  ...
                  />
                  <AgentIcon agentId={agent.id} className="h-4 w-4 text-slate-300" />
            ...
            <div className="px-5 pb-5 flex gap-2 pt-1">
              ...
              <button onClick={handleConfigure} disabled={selected.size === 0 || running} className="btn-primary flex-1 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center gap-1.5">
                {running ? (
                  <>
